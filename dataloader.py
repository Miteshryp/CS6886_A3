...
class CIFAR10Policy(object):
    ...
    def __init__(self, fillcolor=(128, 128, 128)):
        ...
        # Set of policies used to transform images in a dataset. 
        # The policies are randomly chosen during data construction for image augmentation
        self.policies = [
...
def get_cifar10(batchsize=128):
    trans_t = transforms.Compose([transforms.RandomCrop(32, padding=4),
                                  ...
                                  # Introduces random jitter in the image. Helps the model cut through noise to recognize features
                                  transforms.ColorJitter(brightness=0.4, contrast=0.4, saturation=0.4),

                                  # Random Filter based augmentation applied here
                                  CIFAR10Policy(),

                                  # Converts 20% images to greyscale images. 
                                  transforms.RandomGrayscale(p=0.2),

                                  # Converts Data from (W, H, C) to (C, H, W) format, which is tensor format of PyTorch
                                  transforms.ToTensor(),

                                  # Since data is now in (C, H, W), We find mean per channel value for each channel
                                  # mean (R, G, B) in cifar = (129.3, 124.1, 112.4)
                                  # std (R, G, B) = (68.2,  65.4,  70.4)
